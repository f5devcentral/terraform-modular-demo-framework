...
for x in `seq 1 120`; do
    site_state=$(curl --location --request GET $URL -H "Authorization: APIToken $VOLTERRA_TOKEN" -H "content-type: application/json" -s |jq -r '.system_metadata.initializers.pending | length')
